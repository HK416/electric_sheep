...
use es_ir::learning::{
    ActionExecutionMode, ArchKind, DiffusionScheduler, HeadKind, LearningGraph, LearningNode,
...
use crate::lower::torch::diffusion_schedule;
...
/// `_DdpmHead.forward`: the reverse loop, descending in `t`.
pub fn ddpm(
    ck: &Checkpoint,
    cond: &[f32],
    noise: &[f32],
    ...
    fn tier4(kind: HeadKind, name: &str) {
        ...
        let want = match kind {
            HeadKind::Diffusion { n_steps, scheduler } => {
                ddpm(&ck, &cond, &x_t, n_steps, scheduler)
            }
