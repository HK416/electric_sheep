//! The cross-IR fixture, copied verbatim from `crates/es-ir/tests/cross_fixture.rs`
//! (lines 8-611, through `hex`) as the `es` CLI tests already do: the editor must not invent
//! a second idea of a consistent five-IR bundle. The violation scenarios are left out --
//! this file only needs `Fixture::new()` and the builders it calls, and `Fixture`'s fields
//! are `pub` so the test file next to this module can reach them.
#![allow(dead_code)]
