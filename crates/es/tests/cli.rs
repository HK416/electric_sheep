...
// --- packet M8/S2b: `es policy import-rl` --------------------------------------------------

/// One of the three committed synthetic imports, by directory name.
fn import_fixture(framework: &str, name: &str) -> String {
    train_toml_path(&rl_fixture(&format!("import/{framework}/{name}")))
}
...
fn run_import_rl(framework: &str, docs: &Documents, adapter: &Path, out: &Path) -> Output {
    bin()
        .current_dir(train_root())
        .args([
            "policy",
            "import-rl",
            "--manifest",
            &import_fixture(framework, "import.json"),
            "--weights",
            &import_fixture(framework, "weights.safetensors"),
            "--adapter",
            &train_toml_path(adapter),
            "--task",
            &train_toml_path(&docs.task),
            "--deployment",
            &train_toml_path(&docs.deployment),
            "--out",
        ])
        .arg(out)
        .output()
...
/// The committed reach pair, in `JointPosition` (packet M8/S2b) or in `JointDelta` (M9/T2).
///
/// There is no committed delta *Task* IR: T1 committed `deployment-reach-delta.toml` and
/// mutates the task in memory for its own cross-check, so the one line that differs is
...
/// One observation through the lowered module, as `es eval run` would feed it.
fn infer_state(runtime: &mut es_policy::TorchRuntime, state: &[f32]) -> Vec<f32> {
    let inputs: BTreeMap<String, es_compile::Tensor> = [(
        "state".to_owned(),
        es_compile::Tensor {
            dtype: ElemType::F32,
            shape: vec![state.len() as u64],
            data: state.iter().flat_map(|v| v.to_le_bytes()).collect(),
        },
    )]
    .into_iter()
    .collect();
    let outputs = es_policy::PolicyRuntime::infer(runtime, &inputs).expect("infer");
    outputs
        .values()
        .next()
        .expect("one output")
...
/// Oracle 1 of packet M8/S2b. The three committed fixtures -- one per source framework, each
/// generated by `python/es/import_rl.py --synth <framework>` out of that framework's own
/// native layout -- become two documents, a bundle and a mapping report.
///
/// What is actually being checked is that **nothing on this path needs Python**: the fixtures
/// were produced by the Python half once and committed, and everything after them is Rust.
/// The `TorchRuntime` open at the end is the one step that does need it, and it skips with a
/// printed reason rather than pretending (spec 1.4).
#[test]
fn import_rl_synthetic_three_frameworks() {
    let dir = scratch_dir("import-rl");
    let documents = reach_documents(&dir, false);
    let adapter = rl_fixture("adapter-so101.toml");
    let mut weights = Vec::new();

    for framework in ["playground", "rsl-rl", "rl-games"] {
        let out = dir.join(framework);
        let run = run_import_rl(framework, &documents, &adapter, &out);
        assert_eq!(
            run.status.code(),
            Some(0),
            "{framework}:\n{}\n{}",
        ...
        let said = stdout(&run);
        assert!(said.contains("observation_hash: "), "{said}");
        assert!(said.contains("learning_hash: "), "{said}");

        // The bundle is the strong check: `open` re-validates all four IRs, re-runs the
        // cross-IR pass (`XIR-002` on every StateInput source, `XIR-010` on the contract,
        // `XIR-030`/`XIR-031` on the action space) and recomputes every hash.
        let bytes = std::fs::read(out.join("policy.esb")).expect("policy.esb");
        let bundle = PolicyBundle::open(&bytes).expect("the imported bundle opens");
        assert_eq!(bundle.observation.outputs["state"].ty.shape.dims(), [26]);
        assert_eq!(bundle.learning.policy.contract.action_dim, 6);
        assert_eq!(bundle.learning.policy.contract.horizon, 1);

        // And the two documents on disk are the two the bundle carries, not a second
        // rendering of them.
        let observation = es_ir::serial::observation_from_toml(
            &std::fs::read_to_string(out.join("observation.toml")).expect("observation.toml"),
        )
        .expect("the written Observation IR parses");
        let learning = es_ir::serial::learning_from_toml(
            &std::fs::read_to_string(out.join("learning.toml")).expect("learning.toml"),
        )
        .expect("the written Learning IR parses");
        assert_eq!(observation, bundle.observation);
        assert_eq!(learning, bundle.learning);

        // Spec 14.4's Semantic Mapping Report: one row per joint and per channel.
        let report: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(out.join("mapping-report.json")).expect("the report"),
        )
        .expect("the report is JSON");
        assert_eq!(report["joints"].as_array().expect("joints").len(), 6);
        assert_eq!(report["channels"].as_array().expect("channels").len(), 4);
        for row in report["channels"].as_array().expect("channels") {
            assert!(
                !row["name"].as_str().expect("a name").is_empty()
                    && row["unit"].as_str().is_some()
                    && row["severity"].as_str().is_some(),
                "a report row is missing name, unit or severity: {row}"
            );
        }
        weights.push(bundle.weights.clone());
    }

    // The three fixtures are the same synthetic network written three ways, so the three
    // remapped checkpoints must be byte-identical. This is the one thing the Rust side can
    // say about `import_rl.py`'s three readers without running them.
    assert!(
        weights.windows(2).all(|w| w[0] == w[1]),
        "the three frameworks' imports disagree about the weights"
    );

    let Ok(python) = std::env::var("ES_PYTHON") else {
        println!(
            "SKIP the TorchRuntime open of import_rl_synthetic_three_frameworks: \
             ES_PYTHON is not set"
        );
        return;
    };
    let bytes = std::fs::read(dir.join("playground").join("policy.esb")).expect("policy.esb");
    let bundle = PolicyBundle::open(&bytes).expect("the bundle opens");
    let mut runtime = es_policy::TorchRuntime::new();
    es_policy::PolicyRuntime::load(
        &mut runtime,
        &bundle.learning,
        &es_policy::WeightsSource::InMemory(bundle.weights.clone()),
    )
    .unwrap_or_else(|e| panic!("TorchRuntime::load ({python}): {e}"));
    let out = infer_state(&mut runtime, &[0.1f32; 26]);
    assert_eq!(out.len(), 6, "one action row of six");
...
fn import_rl_delta_fixture() {
    ...
    let delta = reach_documents(&dir, true);
    let position = reach_documents(&dir, false);
    let out = dir.join("delta");
    let run = run_import_rl("playground-delta", &delta, &delta.adapter, &out);
    assert_eq!(
        run.status.code(),
        Some(0),
        "{}\n{}",
    ...
    // Three declarations -- the manifest's, the adapter's and the Task IR's -- and every way
    // two of them can disagree is one `IMP-004` that writes nothing. (A manifest with no
    // `action_kind` at all, like the `playground` fixture S2b committed, is not a
    // disagreement: the adapter alone declares, and it always has to.)
    for (case, documents, adapter) in [
        (
            "the source says increment, the adapter says position target",
            &position,
            &position.adapter,
        ),
        (
            "the adapter says increment, the Task IR says position",
            &position,
            &delta.adapter,
        ),
        (
            "the adapter says position target, the Task IR says increment",
            &delta,
            &position.adapter,
        ),
    ] {
        let bad = dir.join(format!("bad-{}", case.len()));
        let run = run_import_rl("playground-delta", documents, adapter, &bad);
        assert_eq!(
            run.status.code(),
            Some(1),
            "{case}: must be refused, not imported:\n{}",
            stdout(&run)
        );
        assert!(
            stderr_of(&run).contains("IMP-004"),
            "{case}: the refusal must name its code:\n{}",
            stderr_of(&run)
        );
        assert!(
            !bad.join("policy.esb").exists(),
            "{case}: a refusal must leave no half-written bundle behind"
        );
    }
    println!("RAN import_rl_delta_fixture: a JointDelta bundle and three IMP-004 refusals");
}
    ...
    header
        ...
        .map(|(name, entry)| {
            ...
            let values = blob[a..b]
                ...
                .collect();
            (name.clone(), (entry.shape.clone(), values))
        })
        .collect()
}
...
///
/// (a) the importer's numpy -> torch reconstruction against **our runtime**, over the lowered
///     module of the imported bundle: **bitwise** f32 on all 1,000 rows. This is the claim
///     that the import itself -- the transpose, the key remap, the graph, the lowering -- lost
///     nothing;
/// (b) our runtime against JAX's own deterministic actions: max abs error, tier 4 (<= 1e-5).
///     It cannot be bitwise: JAX and torch do not share `tanh` or `exp`.
///
/// Tier (b) runs on `obs_scaled`, not on the packet's `U(-1, 1)` draw. Three channels of this
/// task are constant, so their `obs_std` is brax's 1e-6 variance floor and an off-scale sample
/// normalizes to ~1e6, saturating every `swish`; `docs/api-notes/brax-ppo-so101.md` section 6
/// measures 2.3e-5 there against 1.6e-6 in distribution. The tolerance is kept and the set it
/// is applied to is the honest one.
#[test]
#[ignore = "needs ES_PYTHON with torch and ES_S2B_SOURCE pointing at S2c's artifacts"]
fn import_rl_reproduces_the_source_policy() {
    const NAME: &str = "import_rl_reproduces_the_source_policy";
    let (Ok(python), Ok(source)) = (std::env::var("ES_PYTHON"), std::env::var("ES_S2B_SOURCE"))
    else {
        println!("SKIP {NAME}: set ES_PYTHON and ES_S2B_SOURCE");
        return;
    };
    let dir = scratch_dir("import-rl-source");
    let neutral = dir.join("neutral");
    let python_step = |args: &[&str]| {
        let out = Command::new(&python)
            .current_dir(train_root())
            .arg("python/es/import_rl.py")
            .args(args)
            .output()
            .expect("run import_rl.py");
        assert_eq!(
            out.status.code(),
            Some(0),
            "import_rl.py {args:?}:\n{}\n{}",
    ...
    // 1. the Python half: the pickle/orbax door, opened once (INV-16).
    print!(
        "{}",
        python_step(&[
            "--from",
    ...
    let documents = reach_documents(&dir, declared["action_kind"] == "joint_delta");
    ...
    let run = bin()
        ...
        .args([
            ...
            "import-rl",
            "--manifest",
            &train_toml_path(&neutral.join("import.json")),
            "--weights",
            &train_toml_path(&neutral.join("weights.safetensors")),
            "--adapter",
            &train_toml_path(&documents.adapter),
            "--task",
            &train_toml_path(&documents.task),
            "--deployment",
            &train_toml_path(&documents.deployment),
            "--out",
        ])
        .arg(&out)
        .output()
    ...
    // 3. the reconstruction and the two 1,000-row sets, as one safetensors file.
    let oracle = dir.join("oracle.safetensors");
    print!(
        "{}",
        python_step(&[
            "--reference",
            ...
            &train_toml_path(&neutral),
            "--resolved",
            &train_toml_path(&out.join("mapping-report.json")),
            "--npz",
            &format!("{source}/oracle-1000.npz"),
            "--out",
            &train_toml_path(&oracle),
        ])
    );
    let tensors = oracle_tensors(&oracle);
    let (obs_shape, obs) = &tensors["obs_normalized"];
    let (_, reference) = &tensors["reference"];
    let (_, jax) = &tensors["actions_scaled"];
    let (rows, dim) = (obs_shape[0] as usize, obs_shape[1] as usize);

    // The action tail, so tier (b) can compare in the source's own normalized units: our
    ...
    let adapter = es_data::rl_import::Adapter::parse(
        &std::fs::read_to_string(&documents.adapter).expect("the adapter"),
    )
    .expect("the adapter parses");
    let scale = adapter.action.scale.clone().expect("the adapter's scale");
    let offset = adapter.action.offset.clone().expect("the adapter's offset");
    ...
    .expect("TorchRuntime::load accepts the imported bundle");

    let action_dim = bundle.learning.policy.contract.action_dim as usize;
    let (mut mismatches, mut worst_bits, mut worst_jax) = (0usize, 0.0f64, 0.0f64);
    for i in 0..rows {
        let ours = infer_state(&mut runtime, &obs[i * dim..(i + 1) * dim]);
        assert_eq!(ours.len(), action_dim);
        for (j, got) in ours.iter().enumerate() {
            let want = reference[i * action_dim + j];
            if got.to_bits() != want.to_bits() {
                mismatches += 1;
                worst_bits = worst_bits.max(f64::from((got - want).abs()));
            }
            // Back to the source's normalized action: `ctrl = offset + scale * a`.
            let normalized = (f64::from(*got) - offset[j]) / scale[j];
            worst_jax = worst_jax.max((normalized - f64::from(jax[i * action_dim + j])).abs());
        }
    }
    println!(
        "{NAME}: {} , {rows} rows x {action_dim}\n  (a) reconstruction vs our runtime: \
         ...
         policy_hash {}",
        documents.task.display(),
        hex(bundle.learning.policy.weights.hash()),
        bundle
            .manifest
            .hashes
            .policy
            .map_or_else(|| "-".to_owned(), |h| hex(&h)),
    );
    assert_eq!(
        mismatches, 0,
        "tier (a) is bitwise: the import or the lowering changed the function"
    );
    assert!(worst_jax <= 1e-5, "tier (b): {worst_jax:.3e} exceeds 1e-5");
}

/// The `rsl_rl` / `rl_games` half of oracle 3 (server only): a random-weights actor built and
/// saved by the framework's **own** API, imported, and run through our runtime.
///
///     ~/venvs/es-rl-import/bin/python python/es/import_rl.py --synth rsl-rl --native \
///         --out /tmp/rsl-rl
///     ES_PYTHON=... ES_S2B_IMPORT=/tmp/rsl-rl \
///     cargo test -p es --test cli -- --ignored import_rl_matches_the_reconstruction
///
/// `--synth --native` already compared the framework's own `forward` against the importer's
/// reconstruction, bitwise, and printed the result; this compares that reconstruction against
/// our runtime, bitwise. The two links together are the packet's claim. It takes any
/// `import_rl.py` output directory, so the same test serves all three frameworks.
#[test]
#[ignore = "needs ES_PYTHON with torch and ES_S2B_IMPORT pointing at an import_rl.py output"]
fn import_rl_matches_the_reconstruction() {
    const NAME: &str = "import_rl_matches_the_reconstruction";
    let (Ok(python), Ok(neutral)) = (std::env::var("ES_PYTHON"), std::env::var("ES_S2B_IMPORT"))
    else {
        println!("SKIP {NAME}: set ES_PYTHON and ES_S2B_IMPORT");
        return;
    };
    let neutral = PathBuf::from(neutral);
    let dir = scratch_dir("import-rl-framework");
    // rsl_rl and rl_games record no action kind, so the position pair is the one their
    // adapter targets (a delta source would say so in its manifest -- M9/T2).
    let documents = reach_documents(&dir, false);
    let out = dir.join("bundle");
    let run = bin()
        .current_dir(train_root())
    ...
    let reference = Command::new(&python)
        ...
        .output()
        .expect("run import_rl.py --reference");
    assert_eq!(
    ...
    let action_dim = bundle.learning.policy.contract.action_dim as usize;
    let (mut mismatches, mut worst) = (0usize, 0.0f64);
    for i in 0..rows {
        let ours = infer_state(&mut runtime, &obs[i * dim..(i + 1) * dim]);
        for (j, got) in ours.iter().enumerate() {
            let expected = want[i * action_dim + j];
    ...
    println!(
        "{NAME}: {} -> {rows} rows x {action_dim}, {mismatches} mismatching values (max abs \
         {worst:.3e}), weights_hash {}",
        neutral.display(),
        hex(bundle.learning.policy.weights.hash()),
    );
