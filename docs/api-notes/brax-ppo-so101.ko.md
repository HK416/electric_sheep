...
E4는 두 번 읽어야 한다. 이것이 실행 자체를 가능하게 만든 편집이며(§3의 NaN), 동시에 가장 큰
거동 격차다: 여기서 학습한 정책은 큐브를 한 번도 느껴본 적이 없다. S4c가 커밋된 씬의 우리
런타임에서 이 정책을 재생하면 큐브와의 접촉은 처음 겪는 일이고, 큐브가 밀려나는 것은 버그가
아니라 예상된 결과다.
