...
| `tan` | `|x| ≤ 6.5`, 극점을 벗어난 영역 | `sin`/`cos`와 **동일한** `reduce_quadrant` — 축소는 하나, 잘못될 곳도 하나. `tan`은 주기가 π이므로 `quadrant & 1`만 중요하다: 짝수 사분면은 커널을 반환하고, 홀수는 `-1.0 / kernel`을 반환한다. 커널은 Cephes `tanf.c`의 `r²`에 대한 자체 6항 minimax이며, `sin(x)/cos(x)`가 **아니다**: 그 몫은 3 ULP(커널 오차 두 개에 나눗셈까지)로 목표를 넘긴다. `tan'(x) = 1 + tan²(x)`가 축소의 잔차 인자 오차를 증폭시키므로 정의역은 `sin`/`cos`보다 좁다 — 아래 표 참고. |
...
`tanf.c`, `expf.c`, `logf.c`, `atanf.c`)의 단정밀도 minimax 계수이며 퍼블릭 도메인이다. 이
계수들은 위에 나열한, 여기서 쓰이는 것과 정확히 같은 축소된 정의역에 대해 Remez 교환
알고리즘으로 피팅된 것이다. 반올림도, 재유도도 없이 소수 리터럴 그대로 옮겨 적었으므로,
Rust 파일과 Slang 파일을 리터럴 단위로 비교할 수 있다 (P09). `coeffs.rs`에서
...
| `tan` | `[-1.5, 1.5]`와 `[-6.5, 6.5]` (주 정의역) | 2 |
| `tan` | `[-1e3, 1e3]` (기록용, 상한 **아님**) | 3 |
| `exp` | `[-88, 88]` | 1 |
...
`tan`은 주 정의역이 `sin`/`cos`의 전체 범위가 아닌 유일한 함수다. 그 스윕은 정의역이
아니라 크기로 필터링한다 — `f64` 참조값이 `1e3`을 넘는 지점은 극점의 ~1e-3 이내이므로
건너뛴다 — `tan`의 증폭 계수가 `tan'(x)/tan(x) = 1/tan + tan`이기 때문이다. 그래서 `sin`
에서는 0.1 ULP인 축소 오차가 `|x| ≈ 600`의 `tan`에서는 3 ULP가 되고, 극점에서는 무한이
된다. 주장하는 상한은 `|x| ≤ 6.5`에서 2 ULP이며, 더 넓은 스윕은 기록용으로만 출력하고
아무것도 단언하지 않는다. `es-render`는 `tan`을 상한에서 충분히 여유 있는
`fovy/2 ∈ (0, π/2)`에서만 사용한다.
...
**Target / Status: 미검증 (unverified)**이다.

## Rust ↔ Slang 비트 동등성

`slang/approx.slang`은 Rust 소스를 한 줄 한 줄 그대로 옮긴 것이다. 유닛 테스트가 두
파일에서 상수 선언을 파싱해 이름/리터럴 쌍이 바이트 단위로 동일한지 검사한다. 이 둘이
실제 GPU에서 **비트 단위로 동일한 결과**를 내는지는 **Target / Status: 미검증
(unverified)**이다 — §3.4의 실행 모드가 설정된 Vulkan 디바이스가 필요하며, 이는 M2
작업이다 (P09의 acceptance는 리터럴 비교만을 다룬다).
