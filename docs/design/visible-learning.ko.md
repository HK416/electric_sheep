...
### 7.35 만든 대로 (M9/T3): 절대 공간 곁의 증분 공간

플랜 T(§28.12)는 `JointDelta` 행동 공간에서 PPO가 절대 공간에서보다 덜 클램프하고 더 많이
배우는지 물었다. 측정(`docs/design/rl-continuation.md` 7절, "T3"): 4,000 반복에서의 홀드아웃
reach `success_rate`, 시드 3개 — 절대 **0.4167**, 처음부터 학습한 증분 **0.1042**, 임포트된
brax 증분 정책은 이어붙이기 전후 모두 **0.0**. 증분은 `violation.velocity`를 전부 없앴고
클램프를 `violation.position`으로 옮겼다: 적분된 목표가 위치 soft 엔벌로프에 맞닿아 매 틱
