...
열려 채점된 0.820에 **약 3 밀리라디안** 못 미치고, 에피소드 끝에는 다시 −0.009 rad로
닫힌다. 즉 놓기는 이제 정책이 하는 일이고, 놓기와 *채점된* 놓기를 가르는 것은 집게 여행의
마지막 3 밀리라디안과 그 상태를 유지하는지 여부다.
...
그 안에 안정되어 있으며, 그중 하나에서는 집게가 술어에 3 밀리라디안까지 다가간다. 미해결 질문
...
2. **다른 다섯 스위트가 문서에 있으면 nominal 스위트의 숫자가 움직인다.** 50,000에서 nominal만
   ...
   모든 숫자는 측정에 쓰인 문서와 함께 적혀 있다. 그것이 열린 질문 27이다.
...
22. **놓기는 관측 가능하지 않고, 기억 없는 정책이 그것을 보게 만들 수는 없다**
    ...
    **답함 (V17, 7.25절): 기본값을 택했고, 부분적으로 통했다.** 선언된 5 Hz 재계획을 두 경로
    ...
    채점된 성공을 막는 항은 오직 그리퍼이고, 가장 아깝게 놓친 경우는 채점된 0.820에 대해
    0.817 rad — **3 밀리라디안** — 까지 열렸다가 다시 닫힌다. **이제 (ii)가 다음 변수다**:
...
26. **어느 시연 200개가 "현재 시연"이고, 내보내기는 무엇을 가리키는가** (7.27절, V19).
...
27. **다른 스위트가 문서에 있으면 한 스위트의 숫자가 움직인다** (7.27절, V19).
