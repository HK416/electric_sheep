...
**Reviewer's note (orchestrator, 2026-09-16).** Two caveats travel with the 13/16. First, every
evaluation in this section ran against the deployment as it stood on V19's base commit
(`acceleration_max = 20`), so `envelope_violation_rate` 0.95–0.98 and the watchdog latches are the
pre-V18 envelope; the fixture is now 80 (section 7.28) and the w13 checkpoint is re-measured on the
committed document in section 7.29. Second, 60,000 steps was chosen by its held-out score after
40k/50k/60k/100k were all evaluated on the same sixteen seeds; there is no separate validation
split, so the 0.8125 carries selection on the evaluation seeds and the six-suite sweep's nominal
0.75 is the more conservative number. Neither caveat touches the thesis result, which is exact at
every checkpoint.
...
27. **Which 200 demonstrations are "the current demonstrations", and what does the export point
...
28. **A suite's numbers move when the other suites are in the document** (section 7.27, V19).
