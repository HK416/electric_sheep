...
## 16. 전체 루프를 살아 있는 채로: 곡선과 신경망이 보는 것 (§23.1, §23.3, §13.1, M7/E7)

§13은 평가 하나를 지켜봤다. 여기서 지켜보는 것은 *루프*다: 수집, 전문가 관문, 학습, 평가를 그
일을 하는 명령이 발행하고 이미 있는 탭이 그린다. 소유자 메모, 2026-09-21, 로컬 사이클 시연 뒤:
*에디터에서 손실 곡선 같은 데이터를 보여 달라*, 그리고 *학습 과정에는 렌더링된 화면이 없다 —
그것이 아쉽다.*

### 한 주소, 네 단계

`es loop cycle --telemetry <주소>`는 첫 단계 전에 소켓 **하나**를 바인드하고 같은 퍼블리셔를
프로세스 안의 모든 단계에 넘긴다. 모든 스트림 1 이벤트가 `stage` 필드를 지니고,
`stage.begin` / `stage.end`가 각 단계를 그 벽시계 시간과 함께 감싼다. 그래서 에디터는 한 번
붙어서 루프 전체를 본다. 와이어 모양과 단계 이름과 다섯 스트림은
`docs/design/telemetry-protocol.ko.md` §9.1에 있다. 와이어가 있어야 할 곳이 거기다.

| 스트림 | 에디터가 그것으로 하는 일 |
|---|---|
| 1 | `LiveRun`: `cell.*`와 `episode.*`는 행이 되고 `stage.*`는 스트립이 된다. `TrainView`: `train.begin`과 `checkpoint` |
| 2 | `LiveRun::timeline` — 고른 행의 안전 스트립 |
| 3 | `TelemetryModel::metrics` — §12.4 표 |
| 4 | 실행의 카메라 이미지, **또는** 학습 실행의 샘플 텐서 |
| 5 | `TrainView` — 손실, 학습률, 처리량 |

**수집된 에피소드는 같은 표의 행이다.** `LiveRun`은 `episode.begin` / `episode.end`를 평가의
셀이 되는 것과 같은 `CellRow`로 접는다: 이름은 `episode-<NN>`, `suite` 열에는 그것이 온 단계가
들어가고, 스트립은 같은 `StepEvent` 비트를 같은 방식으로 접은 것이다. §13의 "표 하나, 두 끝"이
세 번째로 늘어난 셈이다: Run 탭은 여전히 표 하나, 스트립 하나, 선택 하나이고, 이제 한 행이
...
`checkpoints: Vec<(u32, String)>`와 마지막 샘플 이미지와 트레이너 자신의 `samples_per_s`를
보관하며, `eta(total_steps)`에 답한다.

`plot(series, log)`가 그리기를 결정이 아니게 만든다: **단위 정사각형 안의 점들**과, 그것을
정규화한 기준이 된 `min`/`max`를 *그 계열의 원래 단위로*, 그리고 체크포인트 표시를 `x`로 돌려준다.
`app.rs`는 사각형을 매핑하고 창 위로 `line_segment`를 부른다 — 서른 줄쯤, 의존성 없음, 그리고
테스트가 닿지 못하는 것은 셸 안에 없다. 그 안의 의도된 규칙 둘:

- **로그 스케일은 로그를 취할 수 없는 것을 버린다.** 정확히 `0`인 손실에는 로그가 없다. 그것은
  아닌 값으로 고정되는 대신 그려지는 집합에서 빠지고, 보고되는 범위는 계열의 원래 단위로 남아
  두 축 이름이 지수가 아니라 손실로 읽힌다.
- **평평한 곡선은 가운데에 그린다.** 0으로 나누지 않는다.

`eta`는 **프로듀서의** 시계로 잰다 — 첫 진행 프레임과 마지막 진행 프레임의 `wall_ns`를 그 사이
스텝 수로 나눈 것이다. 늦게 붙은 에디터나 창 관리자가 다시 그리기를 멈춘 에디터가 실행에 없는
속도를 지어내서는 안 된다. 점이 둘이 되기 전에는 `None`이고, 총 스텝에 이르면 다시 `None`이다:
끝난 실행에는 추정이 없다. 총 스텝은 레시피 자신의 `[run] steps`이고, 파이썬이 시작하기 전에
`train.begin { total_steps }`로 한 번 말해지므로 추정은 첫 진행 줄부터 존재한다.

### 샘플 이미지: 렌더가 아니라 텐서

곡선 옆에 있는 것은 **지금 묶음의 이미지 입력 하나, 증강을 거친 뒤의 것** — 장면을 새로 그린
것이 아니라 신경망이 맞춰지고 있는 바로 그것이다. `train_act.py --sample-every N`이 손실 곡선
옆에 `Rgb8`로 쓰고 경로를 찍으며, `es train`이 그 파일을 읽어 스트림 4로 발행한다. 역정규화는
`clamp(v, 0, 1) * 255`이고 데모 체인의 정확한 역이며, 가정되는 대신 **사이드카에 적힌다**.
컨트랙트도 베이크 매니페스트도 정규화의 수를 지니지 않기 때문이다(telemetry-protocol §9.1이
그 천장을 말한다).

그것이 소유자의 두 번째 문장에 대한 답이다. 학습 과정에는 여전히 *렌더링된* 화면이 없다 —
학습 중 롤아웃은 없다 — 하지만 더 이상 눈이 먼 것은 아니다: 곡선이 내려가고, 체크포인트가
포장된 자리에 표시가 생기고, 그 옆의 그림이 그동안 신경망이 보고 있는 것이다.

### Launch 패널이 루프를 시작하고 그것에 붙는다

이제 세 종류 모두가 세 텔레메트리 칸(`--telemetry`, `--telemetry-token`,
`--telemetry-image-every`)을 지닌다. 셋 다 발행하기 때문이다. 그래서 §14의 "`attach()`는 모든
`es train`과 모든 `es loop cycle`에 대해 `None`"은 더 이상 사실이 아니다: 전체 루프에서
**시작**을 누르면 사이클의 한 주소로 스스로 전화를 건다. E5 이후 평가에서 눌렀을 때와 똑같이.
`--telemetry-image-every`는 단계마다 하나의 뜻을 가진 하나의 수다 — 수집과 평가에서는 N 제어
틱마다의 관측 이미지, 학습에서는 N 옵티마이저 스텝마다의 샘플 텐서.

`tests/golden/editor/launch-{train,cycle}.txt`는 `ES_GENERATE_GOLDENS=1`로 한 번 다시 만들었다.
픽스처 형식이 채우는 세 텔레메트리 칸**만큼만** 달라지고 `launch-eval.txt`는 그대로다.

### 게이트 9: 발행이 수집과 학습에 드는 값 (§28.7, §23.4)

§13과 같은 모양이다: 양쪽 각각 세 번, **번갈아**(plain, telemetry, plain, …), 모든 스트림을
빨아들이는 구독자 하나를 붙인 채, 릴리스 빌드, 우분투, RTX 4090.

| | `es train`, 2,000 스텝 | `es loop collect`, 8 에피소드 |
|---|---|---|
| `--telemetry`의 비용 | **`Target / Status: unverified`** | **`Target / Status: unverified`** |

**첫 시도는 인정되지 않으며, 그 이유는 적어 둘 값이 있다.** `es train`은 데모 베이크에서 양쪽
각각 세 번 돌았고(2,000 스텝, batch 8, `--resident-gpu`, `cuda`), 벽시계는 plain 102.8 / 103.9 /
111.6 초, `--telemetry --progress-every 10 --sample-every 100` 쪽 103.8 / 107.0 / 109.9 초가
나왔다. **그러나 구독자가 대부분의 시간 동안 거기 없었다.** E4의 드레이너(`e4-drain.py`)는 소켓에
5초 타임아웃을 남겨 두는데, 평가는 틱마다 무언가를 발행하므로 그것이 터진 적이 없었다. 반면
*학습* 실행은 첫 진행 줄이 나오기 전 `es dataset bake` 동안 — 1분쯤 — 조용하므로 드레이너가
`TimeoutError`로 죽었고, 실행 자신의 마지막 줄은 `closed after 0 frame(s) delivered to 0
client(s)`로 읽힌다. 따라서 위 숫자는 빈 팬아웃으로 발행하는 비용의 측정이지 게이트 9의 모양이
아니며, 그렇게 인용하지 않는다.

다시 재는 일은 두 줄이다: `~/artifacts/plan-v/m7-e7/e7-drain.py`의 핸드셰이크 뒤에
`sock.settimeout(None)`, 그다음 `bash ~/artifacts/plan-v/m7-e7/e7-gate9-train.sh`와
`…/e7-gate9-collect.sh`(둘 다 이미 써 두었고 각각 11분쯤). 스크립트와 레시피와 첫 시도의 로그는
오라클 서버의 `~/artifacts/plan-v/m7-e7/`에 있다. 그것을 돌리기 전까지 학습 실행과 수집의 발행
비용은 **unverified**다 — E4의 `+0.16 % release`는 다른 모양의 실행에 대한 측정이고 이 둘에
대해서는 아무것도 말하지 않는다.

### 여기 없는 것

- **노드별 활성값 통계**(§23.3). 낮춰진 모듈에 그것을 위한 훅이 없고, 이 파일이 지어낸 숫자는
  모듈의 것이 아니다.
- **학습 중 롤아웃.** 트레이너는 학습한다. 정책이 움직이는 것을 지켜보는 일은 평가이고, 그것이
  다음 단계다.
- **트레이너를 멈추거나, 한 스텝 보내거나, 조종하기.** 트레이너는 제어 프로토콜을 말하지 않는다 —
