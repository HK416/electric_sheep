...
demonstration count one at a time). **V10's verdict (2026-09-15)**: all three suspects are cleared and the cause is L18's control period. The next packet is V11, and nothing else in plan V moves until its number exists. **Progress (2026-09-16)**: V11 (control period, 0/16) → V12 (pairing, L19, 0/16) → V13 (GroupNorm, L20, the first carry) → V14 (release diagnostic + 200 demonstrations, in flight). One variable at a time, each packet exposing the next defect: contact-model verification (judged by §17.2's backend
