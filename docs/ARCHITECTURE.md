...
| 3 | **V8 external ACT** (done: 0/16, 0/16, 1/16; bitwise equivalence — the model hypothesis is rejected) | Does a policy designed and trained outside (LeRobot ACT, `lerobot-train`) succeed on the same demonstrations through the same harness — and does our runtime reproduce it identically | v3.0 export (+`meta/stats.json`) → `lerobot-train` → the `lerobot.rs` loader → gate-5-style bitwise equality → `es eval run` nominal `success_rate` (the demo's acceptance of 0.5, unchanged) | D |
| 4 | **V9 showcase render** (done: four expert successes, 1280×720 mp4) | Is there a video a person can watch | Replay-rendering the recorded state trajectory at the observation resolution is bit-identical to the recorded frames, and a 1280×720 H.264 mp4 comes out (the expert's 8/8 first) | B |
...
demonstration count one at a time). **V10's verdict (2026-09-15)**: all three suspects are cleared and the cause is L18's control period. The next packet is V11, and nothing else in plan V moves until its number exists. **Progress (2026-09-16)**: V11 (control period, 0/16) → V12 (pairing, L19, 0/16) → V13 (GroupNorm, L20, the first carry) → V14 (200 demonstrations, held-out carry 9/16, no release) → V15 (the release in the data and the 0.85 predicate, 0/16) → V16 (the frozen-input fixed point diagnosed, open question 23) → **V17 (both paths honour the declared 5 Hz re-plan; V15's 40k checkpoint re-measured without retraining: the first 4 releases in 48 episodes, held-out 1/16 = 0.0625 — L21, L22, L23)**. V15's checkpoint is still the best policy and every evaluation number in sections 7.19–7.24 is re-dated. → **V18 (envelope ablation: `acceleration_max` 40 gives held-out 0.625 — the first time plan V clears the 0.5 acceptance, L23)**. The fixture decision, the full-suite sweep and the showcase videos come next. One variable at a time, each packet exposing the next defect: contact-model verification (judged by §17.2's backend
