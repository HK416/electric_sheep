...
| L22 | 평가기의 추론 지연 모델 | §12.3 | **V17이 측정(열린 질문 24)**: 수집은 `RuntimeHints::expected_latency_ms`를 `AsyncInference`로 모델링해 첫 청크가 tick 1에 닿고 평가기는 tick 0에 실행한다. 같은 시드의 `qpos‖qvel` 궤적이 tick 1에서 갈라진다(케이던스는 두 경로 모두 120틱에 12회로 동일). 사다리 5의 패킷이다 |
