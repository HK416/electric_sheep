...
| L19 | 관측·행동 짝짓기 | §13.2, LeRobot 규약 | **V10이 재고 V12가 고쳤다(2026-09-16)**: `Env::step`이 물리를 전진시킨 *뒤*의 상태를 그 행동과 짝지어 기록하고 프레임도 같은 사후 상태에서 찍었다. 전문가와 평가기는 행동 *전* 관측으로 추론하므로 학습은 (s_{t+1} → a_t), 추론은 (s_t → a_t)였다. 5 ms에서는 0.7 mrad의 잡음, 50 Hz에서는 틱당 0.04 rad. 수정: 스텝 전 상태를 행에 기록하고 프레임·궤적도 같은 순간으로(visible-learning.md 7.20). 페어링 최소가 k=0에서 k=+1로 정확히 한 행 이동, 위반율 0.27 → 0.04. 성공률은 움직이지 않았다 |
| L20 | 학습된 함수 ≠ 배포된 함수 | §8.3, §2.4 | **V13(2026-09-16)**: 처음부터 학습하는 ResNet18을 BatchNorm으로 lowering했는데 lowering이 단일 샘플이라 `train_act.py --batch 8`은 샘플 하나씩 8번 forward를 누적한다 — BatchNorm이 N=1 통계로 학습되고(사실상 InstanceNorm) 추론은 `eval()`의 running 통계를 쓴다. 같은 학습 에피소드에서 청크 L1이 train() 0.011 대 eval() 0.031–0.039(정지 기준선 0.048); running 통계 재보정으로도 0.029. V2–V12의 모든 IR 그래프 정책이 이 결함 위에서 측정됐다. 결정: 처음부터 학습하는 백본은 `GroupNorm(32)`으로 lowering(Diffusion Policy 선례; train()==eval() 비트 동일이 오라클; visible-learning.md 7.21, learning-lowering.md 5.1). 결과: 처음으로 큐브를 잡아 들어 올려 통까지 운반(학습 시드 3/16, 놓지 않아 timeout), 큐브를 움직이는 에피소드 0 → 14–16/16. held-out은 아직 0/16 |
| L21 | 플레인 워치독의 시계 단위 | §9.4, §12.1 | **V17(2026-09-16)**: `SafetyPlane`의 `period_us`는 *제어* 주기인데 수집·평가 두 루프가 모두 *시뮬* 틱(V11 이후 제어 스텝당 4틱)을 `now`로 넘겼다. 매 제어 틱마다 청크가 도착하는 동안은 `accept`가 먼저 `last_chunk_tick`을 찍어 간격이 항상 0이라 보이지 않았고, `rate.inference`를 지키자마자 `inference_deadline`이 28,800틱 중 25,920틱에서 발화했다. 수정: `DomainRunner`는 자기 `control_tick`, `es_eval::runner`는 에피소드 스텝을 시계로(`es-safety` 불변). 5 Hz 재계획은 도착 간격이 최대 180 ms이므로 데모의 `inference_budget`·워치독을 40 → 240 ms로(INV-12: 넓히되 끄지 않는다; 안전 한계·`stale_observation` 80 ms·수용 임계값 0.5 불변) |
| L22 | 평가기의 추론 지연 모델 | §12.3 | **V17이 측정(열린 질문 24)**: 수집은 `RuntimeHints::expected_latency_ms`를 `AsyncInference`로 모델링해 첫 청크가 tick 1에 닿고 평가기는 tick 0에 실행한다. 같은 시드의 `qpos‖qvel` 궤적이 tick 1에서 갈라진다(케이던스는 두 경로 모두 120틱에 12회로 동일). 사다리 5의 패킷이다 **M7/T7가 수정(2026-09-16)**: `es_eval::runner`가 같은 `latency_ticks`로 `es_env::AsyncInference`를 구동; 한 시드의 두 경로 `qpos ‖ qvel` 궤적이 120틱 전부 동일. 선언된 지연 아래서 V19b의 ACT는 여전히 통과(held-out 0.9375)하지만 IR 그래프 정책은 0.0625로 떨어지고, 임포트된 ACT의 *선언된* 200 ms가 이제 결과를 좌우한다(visible-learning.md 7.30, 열린 질문 32) |
| L23 | 선언된 케이던스 아래의 엔벌로프 | §9.3 | **V17(열린 질문 25)**: 청크의 0..9행이 순서대로 실행되자 정책의 `envelope_violation_rate`가 0.998 — `acceleration_max = 20 rad/s²`는 틱당 스텝 변화 0.008 rad를 허용하는데 정책의 행 간 이동은 0.02–0.03 rad — 이고 `EnvelopeViolationRate` 워치독(`max_frac 0.9`)이 실행의 약 1/10에서 폴백을 래치한다. 전문가는 0.29–0.30(V6b의 0.48–0.55에서 하락). 새 명령이 아니라 시연 자체의 명령 스트림이 평균 없이 플레인에 닿는 것이다. 열린 결정: (a) 서보가 실제로 내는 가속도로 넓힘(Deployment IR 결정), (b) delta action space, (c) 보고만. **V18이 (a)를 잰 결과(2026-09-16, 픽스처 불변, 재학습 없음)**: `acceleration_max` 20 → 40이면 held-out success 0.0625 → **0.625**, 80이면 0.625/0.625(학습/held-out)이고 위반율 0.998 → 0.90 → 0.56, 폴백 래치 2,690 → 767 → 53틱. 속도까지 4.4로 넓히면 `action_rate.first_diff_max` 0.08 rad에 걸려 오히려 나빠진다(0.4375). 40은 로터 토크의 38 %, 80은 76 %(스톨 2.94 N·m 기준)를 쓴다. **결정(오너, 2026-09-16): 80 채택**, `velocity_max` 불변; 재유도는 픽스처 주석에, 결정문은 visible-learning.md 열린 질문 25에(패킷 V18) |
| L24 | 부하 아래의 `execution_hash` | §5.3 | **V18b(2026-09-16, 열린 질문 26)**: 같은 GPU에서 렌더가 도는 동안 재현한 평가가 리포트·`evaluation_hash`는 비트 동일한데 `execution_hash`만 달랐다. 실행 시점에 계산되는 슬롯(`runtime`, `hardware_capability`)이 측정값을 읽는다. 출처 문제이지 정확성 문제가 아니며, 슬롯을 선언값으로 고정하고 단독/경합 쌍을 오라클로 두는 패킷이 필요하다 |
...
| 3 | **V8 외부 ACT** (완료: 0/16·0/16·1/16, 등가성 비트 동일 — 모델 가설 기각; V19/V19b: V15 시연으로 재학습한 같은 ACT가 커밋된 문서에서 held-out 15/16 — 논지 충족) | 외부에서 설계·학습된 정책(LeRobot ACT, `lerobot-train`)이 같은 시연·같은 하네스에서 성공하는가, 그리고 우리 런타임이 그것을 동일하게 재현하는가 | v3.0 내보내기(+`meta/stats.json`) → `lerobot-train` → `lerobot.rs` 로더 → 게이트 5식 비트 동등 → `es eval run` nominal `success_rate`(데모 acceptance 0.5 그대로) | D |
| 4 | **V9 쇼케이스 렌더** (완료: 전문가 성공 4편 1280×720 mp4; V18b에서 정책의 홀드아웃 성공 2편 + 16편 모자이크) | 사람이 볼 수 있는 영상인가 | 기록된 상태 궤적의 재생 렌더가 관측 해상도에서 기록 프레임과 비트 동일하고, 1280×720 H.264 mp4가 나온다(전문가 8/8부터) | B |
| 5 | **평가 지연 모델링** (V17 측정: 두 궤적이 tick 1에서 갈라짐, L22) | 평가와 수집이 같은 시간 규약을 쓰는가 | `cargo test -p es-eval`: 지연이 선언된 배포에서 tick 0이 chunk underrun | B |
...
| 15 | **M5 리뷰와 설계 노트 증류** (완료: `docs/reviews/M5.ko.md`, 후속 R1–R10) | 기록이 사양으로 돌아왔는가 | `docs/reviews/M5.md`가 존재하고 `cargo xtask check-spec-refs`가 통과한다 | A |
...
시연 수를 하나씩). **V10의 판정(2026-09-15)**: 세 용의자는 모두 무죄이고 원인은 L18의 제어 주기다. 다음 패킷은 V11이며, 그 결과가 나올 때까지 플랜 V의 다른 어떤 것도 움직이지 않는다. **경과(2026-09-16)**: V11(제어 주기, 0/16) → V12(짝짓기, L19, 0/16) → V13(GroupNorm, L20, 첫 운반) → V14(시연 200편, held-out 운반 9/16·놓기 0) → V15(데이터의 놓기와 술어 0.85, 0/16) → V16(고정 입력 부동점 진단, 열린 질문 23) → **V17(선언된 5 Hz 재계획을 두 경로가 지킴, 재학습 없이 V15 40k 재측정: 48편 중 첫 놓기 4, held-out 1/16 = 0.0625 — L21·L22·L23)**. V15의 체크포인트가 여전히 최고이며 7.19–7.24절의 평가 숫자는 모두 재측정 대상이다. → **V18(엔벌로프 ablation: `acceleration_max` 40에서 held-out 0.625 — 플랜 V가 처음으로 수용 임계값 0.5를 넘었다, L23)**. → **픽스처 80 채택 + V18b(전체 6스위트 스윕 `passed = true`, nominal 0.625, 쇼케이스 mp4 3편) — 플랜 V의 수용 기준이 커밋된 문서 그대로에서 처음 성립했다(visible-learning.md 7.28).** → **V19(LeRobot의 실제 ACT를 V15 시연으로 재학습, 비트 동일 재현, held-out 13/16 at 20) → V19b(같은 체크포인트를 커밋된 문서로: 학습 16/16, held-out 15/16, 6스위트 `passed`, 폴백 0)**. M5 리뷰: `docs/reviews/M5.ko.md`(B-1 스위트 독립성, F-1..F-10 규칙 후보, R1–R10). 한 번에 변수 하나, 각 패킷이 다음 결함을 드러냈다: 접촉 모델 검증(§17.2의 백엔드 비교로 판정), delta action space(§8.5, `es-ir`
...
| 4 | **T7 평가 지연 모델** (M5 R2, L22; 완료 — visible-learning.md 7.30) | 선언된 `expected_latency_ms`에서 틱 0이 chunk underrun인가, 수집·평가 궤적이 마지막 틱까지 일치하는가 | `cargo test -p es-eval` | B |
...
**M7 결과 (2026-09-21, `docs/reviews/M7.ko.md`).** 사다리의 모든 행이 닫혔다 — T1–T8, E1–E5, R1(+R1b)–R4,
U — 그리고 소유자 지시로 사다리 밖의 **E6**(쉬운 말·`i18n` 문자열 표·시스템 CJK 폰트 대체·홈 화면·네이티브
파일 대화상자)가 추가됐다. 위 규칙 셋은 지켜졌다: 커밋된 관측 픽셀은 한 바이트도 움직이지 않았고(모든
렌더 개선은 옵트인 `RenderConfig` 필드), `es train`은 만들어 낸 다이제스트를 쓰지 않았으며, 에디터의 모든
패널은 헤드리스 뷰모델을 먼저 가졌다. 실측(RTX 4090): 20,000스텝 675 s → 137 s(T3); 배치 64 + warmup-cosine이
같은 샘플 수에서 배치 8보다 손실 36 % 낮음, 26 s 대 107 s(T4); 한 사이클(수집 200편 → 전문가 게이트 → 학습 →
평가 → 쇼케이스) 30:19(T2, 그중 평가 20:24); 1280×720 한 프레임 80 → 5.2 ms 비트 동일(R1), `Full` 룩 9.9 ms(R2),
PT NEE 64 spp 262 ms·4 spp 누적 29.8 ms(R3·R4), ReSTIR 편향 −1.76 % → +0.10 %; 라이브 텔레메트리 +0.16 %(E4).
**재측정(U)**: T7 지연 모델 아래 커밋된 문서에서 U0(T3+T4) held-out 0.25, U1(사전학습) 발산(NaN, 4,517스텝),
U2(증강) 0.1875, **U3(사전학습+증강) 0.5625 — 데모 수용 기준을 통과한 첫 IR 그래프 정책**(같은 평가기에서
V18b는 0.0625). 위의 중단 규칙은 문자 그대로 읽으면 발동하고(0.5625 < 0.625) 같은 조건으로 읽으면 반대이며,
어느 쪽을 택할지는 리뷰의 사람 결정 첫 항목이다. T8의 에피소드 분할은 2.0×/2.2×로 측정됐으나 배송되지 않았다:
...
| 3 | **S4e 관측 캡처가 관절 속도와 몸체 포즈를 읽는다** | `JointState` 채널이 `quantity`(부재 = Position = 오늘의 해시)를 말하고 `es-eval`의 캡처가 `qvel`과 자유 관절이 아닌 몸체의 `xpos ‖ xquat`을 읽어, 26폭 reach 관측이 `Rollout`에서 백엔드 상태와 비트 동일하게 잡히는가(S4d가 찾은 간극); 그러면 reach 학습(S4b의 미룬 오라클 4)이 목표에 닿는가 | `cargo test -p es-eval capture_reads_qvel_and_body_pose`; `--ignored`: `Rollout`의 26폭 포트가 슬라이스별로 백엔드 상태와 비트 동일; 서버: `training-reach.toml` → `es eval run` `success_rate`(≥ 0.8 목표, 실측 기록) | B |
...
**M8 결과 (2026-09-22, `docs/reviews/M8.ko.md`).** 사다리의 모든 행이 닫혔고 파동 3에 **S4d**(보상 콘: `Expr::Sqrt`·`Source::Xpos`
레인·`Norm{L2}`)와 **S4e**(관측 캡처: `JointState.quantity`·`Capture::Qvel/BodyPose`)가 S4b가 찾은 간극으로 추가됐다. 규칙 넷은
지켜졌고 `es-safety`는 한 줄(창 비우기)만 바뀌었다. 실측(오라클 서버): R1의 `(cell, episode)` 분할은 커밋된 문서에서 jobs 1·2·4가
비트 동일하고 nominal 16편이 125.7 s → 60.5 s(48 %), U3 재측정은 바이트 동일(창은 `max_frac 0.9` 근처에서만 작용한다); S2c는
brax PPO로 우리 장면(접촉 35쌍을 뺀 파생 MJX 장면)에서 reach 정책을 1.00으로 학습했으나 **커밋된 장면에서는 0.00**(테이블을 뚫고
도달); S2b는 brax 재구성 대 우리 런타임 비트 동일, JAX 대비 9.7e-7, rsl_rl·rl_games 비트 동일; S1은 U3 0스텝 비트 동일; S4b는
CPU 백엔드에서 두 번 비트 동일; S4e의 reach 학습은 4,000회에서 held-out **0.5625**로 정점(목표 0.8 미달, 이후 되돌아감); **S4c**:
가져온 정책 0.00, 이어하기 0.00(가중치가 4,000회 뒤에도 비트 동일 — 커밋된 관측에서 tanh가 포화해 기울기가 정확히 0), 같은 구조
처음부터 0.083, 64×64 처음부터 **0.417**(시드 3개). 엔지니어링으로는 통과이고 캠페인의 주장("가져온 정책을 이어 학습")은 입증되지
...
**M9 결과 (2026-09-22, `docs/reviews/M9.ko.md`).** 사다리의 모든 행이 닫혔다. R1: `init_log_std`·엔트로피·스케줄을 하나씩 움직여도 clamp율은
1.00(1,024만 행 중 8행만 안 잘림) — 절대 목표 정책은 잡음과 무관하게 플레인의 속도·가속도 한계 밖에서 움직인다. R6: `graph`·`hash`를
`es-ir-types`로 내려 `es-ir` 6,148 → 5,755줄, 호출자 무변경, 해시 불변. T1: `JointDelta`와 `es-env`의 적분기 하나(상태 = 플레인의
`last_safe_action`), `es-safety` diff 0, 데모 궤적 비트 불변, `Rollout` 증분 ≡ 절대 적분값 비트 동일. T2: brax 증분 정책 성공 1.00(파생
장면), 두 번 비트 동일, 가져오기 비트 동일 / JAX 대비 3.3e-7. **T3**: 증분 공간 처음부터 held-out **0.104**(시드 3개) 대 절대 **0.417**;
가져온 증분 정책은 학습 전후 **0.0**; clamp율은 여전히 1.00인데 속도 위반은 0이 되고 **위치 소프트 엔벌로프**가 매 틱 잡는다(적분된
