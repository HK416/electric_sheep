...
| L2 | 정책 성능 (비전) | 데모 `evaluation.toml`의 `success_rate >= 0.5` | 하네스를 모두 고친 뒤 재측정: nominal **0/16**, 6스위트 **0/96** (50 데모, 96×96, from scratch, 20,000 step; visible-learning.md 7.12 phase 2) |
| L3 | 정책 성능 (특권 상태) | 같음 | V7a phase 2 **측정 중 / pending**. 중단 규칙: nominal `success_rate ≥ 0.5`에 못 미치면 다음 용의자는 모델 크기·학습 길이가 아니라 물리·접촉 모델·전문가 궤적이다 (visible-learning.md 7.14) |
...
| L13 | 사전학습 백본 | §8.3 | `pretrained = true`는 lowering이 거부한다. 사전학습 가중치를 싣는 경로 자체가 없고, §29의 라이선스 행은 아직 도달하지도 않았다 |
...
| 3 | 사전학습 시각 백본 | lowering의 `pretrained` 지원과 safetensors 가중치가 선행 조건이다(L13) |
...
|---|---|---|---|---|
| 1 | **V7a phase 2** (진행 중) | 관측에 답이 들어 있으면 이 그래프는 태스크를 하는가 | `es eval run` nominal 16 시드 → `report.json`의 `success_rate` (중단 규칙 L3) | D |
| 2 | **M5-R1 하네스 선행 규칙** | 같은 결함이 다시 학습 뒤에 발견되지 않게 하려면 | 새 수직 슬라이스 패킷의 acceptance가 전문가-통과 테스트 이름을 담는다 | A |
| 3 | **V7b 비전 스테이지 2** | 데이터와 해상도가 비전 정책을 살리는가 | 같은 스위트, 상태 정책의 수치를 천장으로 비교 | D |
| 4 | **평가 지연 모델링** | 평가와 수집이 같은 시간 규약을 쓰는가 | `cargo test -p es-eval`: 지연이 선언된 배포에서 tick 0이 chunk underrun | B |
| 5 | **데모 acceptance 재고정** | 0.9가 아닌 무엇이 정책의 퇴행을 잡는가 | `es eval run`이 전문가는 통과시키고, 위반율이 전문가 최악값을 넘는 정책은 실패시킨다 | C |
| 6 | **`Cargo.lock` 고정** | `compiler_hash`가 빌드를 재현하는가 | `cargo xtask ci`가 lock 파일과 `--locked` 빌드를 검사 | A |
| 7 | **telemetry flake** | CI가 부하에서도 초록인가 | `cargo test -p es-telemetry transport` 100회 반복 통과 | A |
| 8 | **배치 lowering 벡터화** | 학습 시간을 무엇이 지배하는가 | 40 step 손실 곡선이 현 경로와 바이트 동일하면서 벽시계가 내려간다 | B |
| 9 | **`Env` 에피소드 seek** | 에피소드 단위 병렬이 가능한가 | seek 뒤의 상태가 0..n 재생과 비트 동일 | B |
| 10 | **MJWarp를 평가 경로에** | 백엔드가 정말 교체 가능한가 | `es backend compare --backends mjwarp,mujoco-cpu`가 max \|dqpos\|를 보고하고 두 리포트의 판정이 같다 | C |
| 11 | **미강제 안전 한계**(L6) | 선언된 한계가 살아 있는가 | `cargo test -p es-safety`: EE 속도 한계를 넘는 명령이 `Clamped`로 세어진다 (INV-12 — 넓히되 끄지 않는다) | B |
| 12 | **Python 없는 추론** | 배포 경로가 Python 없이 도는가 | 같은 체크포인트에서 torch 경로와 §8.9 계층 4 동등 | C |
| 13 | **`es-ir` 분할 → provenance 검증기** | 특권 채널이 실기로 새지 않는가 | `Real` 모드 Deployment IR + `sim_` 채널 → 검증 오류 | C |
| 14 | **M5 리뷰와 설계 노트 증류** | 기록이 사양으로 돌아왔는가 | `docs/reviews/M5.md`가 존재하고 `cargo xtask check-spec-refs`가 통과한다 | A |

**사다리에 없는 것, 그리고 그 이유.** 자체 물리 솔버는 §1.9의 축소 1번으로 그대로 잘려 있고, 위
11·13이 그 자리를 대신한다 — 지금 필요한 것은 새 솔버가 아니라 쓰고 있는 솔버의 접촉 모델을
판정하는 일이다. PT·ReSTIR 렌더 경로(§1.9 축소 2번)도 데모에 들어오지 않는다: 96×96 래스터
이미지가 정책을 막고 있다는 증거가 없고, 해상도(3번 사다리)가 먼저다. 에디터·저작 쪽도
마찬가지로 이번 사다리에 없다 — M5가 드러낸 결함 중 UI가 원인인 것은 하나도 없었다. 1·2·5·6·7은
사람 한 명의 판단을 기다리지 않으므로 병렬로 착수할 수 있고(§28.8의 유형 A·B), 3·12·13은 각각
앞 행의 결과나 분할 패킷을 기다린다.

**M6의 모양은 1–3의 결과가 정한다.** 상태 정책이 통과하고 비전이 따라오면 M6은 **실기**다 —
§28.7의 게이트 14·15·16, 로봇 셀·HIL 리그·실물 카메라, 즉 §29의 "Safety Plane 요구사항이 실기와
안 맞음" 행을 닫는 작업이다. 상태 정책까지 실패하면 M6은 실기가 아니라 **물리·전문가·액션
표현**으로 내려간다: 접촉 모델 검증(§17.2의 백엔드 비교로 판정), delta action space(§8.5,
`es-ir` 분할 선행), 전문가 궤적의 재설계. 어느 쪽이든 사다리의 2·4·5가 선행 조건이다 — 하네스를
신뢰할 수 없으면 어느 쪽 결론도 측정이 아니다.
...
| 사전학습 백본 라이선스 | 중간 | §25.2. `base_model.lock` + 배포 시 검사 |
