...
§28.9 rule 3 (one variable at a time), §28.11 (plan S's ladder, which this wave continues; the
next campaign has no spec section yet, and the number this packet was drafted with resolved to
nothing — `check-spec-refs` refused it), §12.4. Review: `docs/reviews/M8.md` S-3 and
